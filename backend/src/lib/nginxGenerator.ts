...
// Ported from frontend src/lib/nginxGenerator.ts so the config the server
// actually deploys matches the preview shown in the UI. Keep buildParts()
// in sync with the frontend file.
//
// IMPORTANT topology fix (see graphScope.ts): a Domain can fan out into
// several independent branches — some through explicit Route nodes, and
// optionally one "root" branch (middleware/backends attached directly to
// the Domain, outside any Route). Each branch gets its OWN location block
// with its OWN upstream/rate-limit/cache, computed from only the nodes
// reachable within that specific branch — never merged with a sibling
// branch's nodes. A Listener can also fan out to multiple Domains, each
// getting its own server{} block.
...
/**
 * Builds the content of ONE location{} block (route-specific or the
 * domain's root "/") from only the nodes reachable within that branch.
 * Returns the lines to go inside `location ... { ... }` plus any top-level
 * (http{}-scope) declarations it needed — its own upstream{}, its own
 * limit_req_zone, its own proxy_cache_path. Nothing here is shared with a
 * sibling branch, which is the fix for branches silently merging/dropping
 * each other's backends.
 */
function buildLocationContent(
  ...
  // scoped to just this location/route (or root branch) — see graphScope.ts.
  // Both use the "pf" log_format (defined in docker/nginx.conf), which
  // includes $upstream_addr so backend-level counts can be attributed
  // correctly when multiple backends sit behind one Load Balancer.
  ...
  if (grpcs.length) {
    ...
    // Root branch with no backend at all (nothing attached directly to the
    // domain besides routes) — don't emit a dangling default location.
...
function buildDomainServer(
  ...
  // Root "/" branch — middleware/backends attached directly to the domain,
  // outside any Route. Rendered whenever it actually has something to
  // proxy to, OR when there are no explicit Routes at all (the minimal
  // Domain -> SSL -> Backend pattern).
  ...
  // Every explicit Route branch, each with its own isolated subtree.
...
/** Full preview: wrapped in http{}/stream{}, for the compile/preview endpoint. */
...
/**
 * Unwrapped fragments meant to be `include`d directly inside the real
 * nginx.conf's http{} / stream{} contexts.
 */
export function generateNginxFragments(workflow: Workflow): { http: string | null; stream: string | null } {
  const { httpTop, httpServers, streamBlocks } = buildParts(workflow);
  const header = `# Generated by ProxyForge for workflow "${workflow.name}" (v${workflow.version}) — do not hand-edit\n`;

  let http: string | null = null;
  if (httpServers.length) {
    const lines: string[] = [header];
    for (const t of httpTop) lines.push(t);
    if (httpTop.length) lines.push("");
    for (const s of httpServers) lines.push(s, "");
    http = lines.join("\n");
  }
  ...
  if (streamBlocks.length) {
    const lines: string[] = [header];
    for (const s of streamBlocks) lines.push(s, "");
    stream = lines.join("\n");
  }
