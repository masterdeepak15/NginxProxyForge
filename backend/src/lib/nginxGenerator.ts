...
// actually deploys matches the preview shown in the UI. Keep the server-
// block-building logic (buildParts) in sync with the frontend file.
//
// Two entry points:
//  - generateNginxConfig(wf)   -> full preview, wrapped in http{}/stream{}
//                                  (used by POST /workflows/:id/compile)
//  - generateNginxFragments(wf)-> unwrapped server/upstream blocks only,
//                                  meant to be `include`d from the real
...
function buildParts(workflow: Workflow): Parts {
  ...
  for (const listener of httpListeners) {
    ...
    // Backend-only addition (not present in the frontend preview generator):
    // per-listener access/error logs so /metrics and /logs can read real traffic.
    srv.push(line(1, `access_log /data/logs/${workflow.id}_${listener.id}.access.log combined;`));
    srv.push(line(1, `error_log /data/logs/${workflow.id}_${listener.id}.error.log warn;`));
    ...
    const effectiveRoutes: Array<WorkflowNode | null> = routes.length ? routes : [null];
    for (const r of effectiveRoutes) {
      ...
      if (grpcs.length) {
        ...
        for (const h of get<HeaderEntry[]>(g, "grpcHeaders", [])) {
          if (h?.name && h?.value) srv.push(line(2, `grpc_set_header ${h.name} "${h.value}";`));
        }
        ...
        for (const h of get<HeaderEntry[]>(first, "proxyHeaders", [])) {
          if (h?.name && h?.value) srv.push(line(2, `proxy_set_header ${h.name} "${h.value}";`));
        }
 ...
 * nginx.conf's http{} / stream{} contexts (the container's nginx.conf
 * does `include /data/nginx/conf.d/http/*.conf;` inside http{}, and
 * `include /data/nginx/conf.d/stream/*.conf;` inside stream{}).
 */
