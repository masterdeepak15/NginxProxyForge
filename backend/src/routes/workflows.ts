...
workflowsRouter.post("/import", (req: AuthedRequest, res) => {
  ...
  if (!config || typeof config !== "string" || !config.trim()) {
    return res
      .status(400)
      .json({
        error: { code: "bad_request", message: "config text is required" },
      });
  }
...
workflowsRouter.get("/:id/versions", (req, res) => {
  const rows = db
    .prepare(
      "SELECT version, updated_at as updatedAt, author, message FROM versions WHERE workflow_id = ? ORDER BY version DESC",
    )
    .all(req.params.id);
  res.json(rows);
});

workflowsRouter.delete("/:id/versions/:version", (req: AuthedRequest, res) => {
  ...
  if (!Number.isInteger(versionNum)) {
    return res
      .status(400)
      .json({
        error: { code: "bad_request", message: "version must be an integer" },
      });
  }
  ...
  if (versionNum === wfRow.version) {
    return res.status(409).json({
      error: {
        code: "conflict",
        message:
          "Can't delete the current version. Roll back to another version first.",
      },
    });
  }

  // A version has no standalone file on disk (snapshots live in the DB), but
  // it may have deployment log entries recorded against it — clean those up
  // too so Deployments history stays consistent with what Version history shows.
  db.prepare(
    "DELETE FROM deployments WHERE workflow_id = ? AND version = ?",
  ).run(id, versionNum);
  db.prepare("DELETE FROM versions WHERE workflow_id = ? AND version = ?").run(
    id,
    versionNum,
  );
  ...
  const restored: Workflow = {
    ...
    version:
      (
        db
          .prepare(
            "SELECT MAX(version) as v FROM versions WHERE workflow_id = ?",
          )
    ...
    return res
      ...
      .json({
        error: {
          ...
          message: "Workflow has topology errors",
          details: errors,
        },
      });
  }
