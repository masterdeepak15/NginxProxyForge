...
function WorkflowEditor() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await dispatch(
        saveWorkflowThunk({ id: workflow.id, nodes: workflow.nodes, edges: workflow.edges }),
      ).unwrap();
  ...
  const handleRollback = useCallback(
    async (toVersion: number) => {
      if (!workflow) return;
      try {
        await dispatch(rollbackWorkflowThunk({ id: workflow.id, toVersion })).unwrap();
        toast.success(`Rolled back to v${toVersion} (saved as a new draft version)`);
